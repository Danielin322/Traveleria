...
2. **Global cache per destination** — yes. One cover photo per destination (slug), shared across every trip/user that picks it. No per-trip copy is stored.
...
8. **Schema shape** — a separate `destination_covers` table, keyed by `slug`, rather than 3 columns duplicated on every `trips` row. This matches decision 2 (the cache is global per destination, not per trip): the cover data is stored once per destination and trips just reference it.

## Required changes

### DB
- Migration `sql/013_destination_covers.sql`:
  ```sql
  ...
  ```
  `destination_covers` holds one row per destination (the global cache from decision 2) — `cover_image_url` is the full public S3 URL (decision 7), `credit_name`/`credit_url` are the optional photographer credit (decision 5), `photo_id` is the Pexels photo id, kept for reference/debugging.
  `trips.destination_slug` is a nullable FK pointing at the cache row for that trip's destination — no duplicated cover data per trip (decision 8). `ON DELETE SET NULL` so a trip never breaks if its cache row is ever pruned.
- Reading a trip's cover means a `LEFT JOIN destination_covers ON destination_covers.slug = trips.destination_slug` in the trip queries in `lambdas/trips/handler.py` (`_get_trips`, and the `RETURNING`/re-select in `_create_trip`/`_update_trip`), and exposing `cover_image_url`/`credit_name`/`credit_url` in `serialize_trip` (`shared/utils.py:55`).
...
- Folder structure inside the bucket: `covers/destinations/<slug>.jpg` + `covers/defaults/travel-cover.jpg`.
...
`https://trip-covers-traveleria.s3.us-east-1.amazonaws.com/covers/destinations/<slug>.jpg`
...
- A shared helper in `shared/` (`shared/cover_image.py`) with the logic: normalize destination to a slug → look up `destination_covers` by slug (the DB *is* the cache) → if missing: search Pexels (`orientation=landscape`, `large` size) → download and upload to S3 → upsert the new row into `destination_covers` → fall back to a fixed `travel-default` slug pointing at `covers/defaults/travel-cover.jpg` on any failure. The helper returns the `slug` to store on the trip.
- Both `_create_trip` **and** `_update_trip` (`lambdas/trips/handler.py`) call this helper and set `trips.destination_slug` to the returned slug, so editing the destination also updates the cover (decision 4).
...
- Trip cards render the cover photo as a thumbnail, plus a small optional attribution line ("Photo by {credit_name} on Pexels", linked to `credit_url`) when a credit is available (decision 5 — no longer a compliance requirement, just a nice touch). There is no separate `TripCard` component — this is inlined directly in `home.tsx`'s `renderTripItem`.

## Status: implemented
All of the above has been built:
- `traveleria-backend/sql/013_destination_covers.sql`
...
- `traveleria-backend/local_server.py`, `deploy_cloudshell.sh`, `requirements.txt`, `.env.example` updated accordingly
- `traveleria/app/(tabs)/home.tsx` (destination autocomplete + cover/attribution rendering)

Remaining setup is infrastructure only (not code) — see the checklist that was given separately: create the `TRIP_COVERS_BUCKET` S3 bucket, upload the fallback image, get a `PEXELS_API_KEY`, add both to your local `.env`, and run `python scripts/init_db.py` to apply the migration.
